...
bool trigger_validation() {
  if ((millis() - fst_detection > time_of_detection) && evaluation) {  //evalua el tiempo de analisis
    ...
    if ((peaks >= 2 && peaks <= averageSpikes) && !spasm_dt){  //evaluar los picos necesarios para enter
      if((promedial_medition/meditions_count) >= average_wink*1.1){
        if(calRMS()>=1900){
          ...
          //Serial.println("EVENTO RECHAZADO rms");
        }
      }
      else {
...
float calRMS() {
  ...
  for (int i = 0; i < sice; i++) {
    suma += bufferms[i] * bufferms[i];
  }
  return sqrt(suma / sice);
}
...
void loop() {
  if (!body_conection(digitalRead(Lo_neg), digitalRead(Lo_pos))) {  //buscar estar bien conectado
    ...
    Serial.print(",");
    Serial.print(calRMS());
    int sig_detect = (calRMS()>=1900);
    int sig_valid = ((promedial_medition/meditions_count) >= average_wink*1.1);
    int sig_reject = ((peaks >= 2 && peaks <= averageSpikes) && !spasm_dt);
    Serial.print(sig_detect);
    Serial.print(",");
    Serial.print(sig_valid);
    Serial.print(",");
    Serial.println(sig_reject);
